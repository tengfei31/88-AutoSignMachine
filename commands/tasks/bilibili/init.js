...
module.exports = async (axios, params) => {
  ...
  if (result.code !== 0 || !result.data.isLogin) {
    ...
    console.info(`获取用户状态成功 用户名：%s，硬币余额：%s，当前等级Lv%s，经验值为：%s，下级经验值为：%s`, userInfo.uname.substr(0, 2) + "********", userInfo.money, userInfo.level_info.current_level, userInfo.level_info.current_exp, userInfo.level_info.next_exp)
    if (userInfo.level_info.current_level < 6) {
      console.info(`距离升级到Lv%s还有: %s天`, userInfo.level_info.current_level + 1, new Number((userInfo.level_info.next_exp - userInfo.level_info.current_exp) / EveryDayExp).toFixed(2))
    }
    return userInfo
  }
}